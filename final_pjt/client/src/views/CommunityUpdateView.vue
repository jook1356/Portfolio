    ...
    <div class="create-box">
      ...
      <form @submit.prevent="updateReview">
        ...
        <div class="movie-and-rating">
          <div>
            <div @click="openModal" class="movie-hash"><i class="bi bi-hash" style="font-size: 1.2rem;"></i><span id="selectedMovie"></span></div> <span class="movie-choose-msg" id="movie-msg" v-if="!movie">영화를 선택해주세요</span>
          </div>
    ...
    getReview() {
      axios({
        ...
        .then((res) => {
          ...
          this.movie = this.review.movie.id
          const pTag = document.querySelector('#selectedMovie')
          pTag.innerText = this.review.movie.title
        })
        .catch((err) => {
          console.log(err)
        })
      ...
      axios({
        ...
        .then(() => {
          ...
          if (this.movie) {
            axios({
              method: 'GET',
              url: `${API_URL}/movies/update_movie_vote/${this.movie}/`,
            })
              .then((res) => {
                const vote_count = res.data.vote_count
                const vote_average = (res.data.vote_average * vote_count + this.vote - this.review.vote) / vote_count
                axios({
                  method: 'POST',
                  url: `${API_URL}/movies/update_movie_vote/${this.movie}/`,
                  data: {
